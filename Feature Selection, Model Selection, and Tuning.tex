...
To compare the Ridge and Lasso, let us first transform our error function (which is a quadratic / convex function) into a contour graph.
	...
	\section{ML Pipeline and Hyperparameter Tuning}


	\resetquestioncounter{}
	\begin{qanda}
		\question{What is the difference between fit, fit\_transform, and transform?}

		\answer{The way we use fit and predict in regression, similarly for functions that transform the data - we have fit and transform:
...
\textbf{fit\_transform} - to first fit the parameters of the function and then transform the data.
	...
	\begin{qanda}
		\question{How do you tune a model using train, test, and validation split?}
	...
	\begin{qanda}
		\question{How do you upgrade the Numpy library?}
...
Anaconda prompt: \textcode{pip install numpy==1.20.3 --user}
}
	\end{qanda}

	\begin{qanda}
		\question{Tuning the model using Grid search is taking a long time to run. What are the recommendations for improving performance?}

		\answer{Tuning a model using grid search usually takes a long time, you can try the following to get more insights

\vspace{\baselineskip}
\textcode{grid\_cv = GridSearchCV(estimator=pipe, param\_grid=param\_grid, scoring=scorer, cv=5, n\_jobs = -1, verbose = 2)}

\vspace{\baselineskip}
n\_jobs = -1 can speed up the tuning process by utilizing all the CPU cores.

verbose = 2 will give you the number of times the model has to be fit so that you will get an idea of how much time will it take
